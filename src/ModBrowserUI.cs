...
namespace ModBrowser
{
	internal static class ModBrowserUI
	{
		...
		{
			ModDownloader.StartNewSongSearch();
		...
		private static void DownloadFullPage()
		{
			...
			foreach (var song in activeSongList.songs)
			{
				...
				MelonCoroutines.Start(ModDownloader.DownloadSong(song.song_id, song.download_url, (songID, success) => { OnDownloadDone(song, success); }));
			}
		}

		public static void AddSongItems(OptionsMenu optionsMenu, APIModList songlist)
		{
			CleanUpPage(optionsMenu);
			activeSongList = songlist;
			optionsMenu.screenTitle.text = "Displaying page " + ModDownloader.page.ToString() + " out of " + songlist.total_pages.ToString();

			var pageHeader = optionsMenu.AddHeader(0, "Listing " + songlist.mod_count.ToString() + " songs");
			optionsMenu.scrollable.AddRow(pageHeader.gameObject);

			AddPageButtons(optionsMenu);

			foreach (var song in songlist.mods)
			{
		...
		private static void AddPageButtons(OptionsMenu optionsMenu)
		{
			...
			var previousPage = optionsMenu.AddButton(0,
				"Previous Page",
				new Action(() => { ModDownloader.PreviousPage(); ModDownloader.StartNewPageSearch(); optionsMenu.scrollable.SnapTo(0); }),
				null,
				null);
			...
			var nextPage = optionsMenu.AddButton(1,
				"Next Page",
				new Action(() => { ModDownloader.NextPage(); ModDownloader.StartNewPageSearch(); optionsMenu.scrollable.SnapTo(0); }),
				null,
				null);
		...
		private static void CreateSongItem(Song song, OptionsMenu optionsMenu)
		{
			...
			if (SongLoadingManager.songFilenames.Contains(song.filename) || ModDownloader.downloadedFileNames.Contains(song.filename) ||
				ModDownloader.failedDownloads.Contains(song.filename))
			{
				if (ModDownloader.failedDownloads.Contains(song.filename))
			...
			var previewButton = optionsMenu.AddButton(1,
				"Preview",
				new Action(() => { MelonCoroutines.Start(ModDownloader.StreamPreviewSong(song.preview_url)); }),
				null,
				null);
		...
		static IEnumerator WaitForSpawningMenu(GameObject panel)
		{
			...
			AddSongItems(panel.GetComponentInChildren<OptionsMenu>(), ModDownloader.modList);
		}
	}
}
